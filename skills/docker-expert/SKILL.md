...
**Windows (PowerShell):** Replace all bash commands with PowerShell equivalents:
- `find . -name "Dockerfile*"` → `Get-ChildItem -Recurse -Filter "Dockerfile*"`
- `docker --version 2>/dev/null` → `docker --version 2>$null`
- `grep` → `Select-String`
- `chmod +x` → skip (not applicable on Windows)
- Use `PowerShell` tool, not `Bash` tool
